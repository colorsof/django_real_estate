...
class IssueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Issue
        fields = [
            'id', 'apartment_unit', 'reported_by', 'assigned_to',
...
class IssueStatusUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Issue
        fields = [
            'apartment', 'reported_by', 'assigned_to', 'resolved_by',
            'title', 'description', 'status', 'priority', 'resolved_on'
        ]

    def update(self, instance: Issue, validated_data: dict) -> Issue:
        if (
            validated_data.get('status') == Issue.IssueStatus.RESOLVED and
            instance.status != Issue.IssueStatus.RESOLVED
        ):
