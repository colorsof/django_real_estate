...
class Issue(TimeStampedModel):
    ...
    resolved_on = models.DateTimeField(null=True, blank=True, verbose_name=_('Resolved On'))
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        if (is_existing_instance and 
            self.assigned_to != old_assigned_to and self.assigned_to is not None):
            self.notify_assigned_user()
            
    def notify_assigned_user(self) -> None:
        try: 
            subject = f"Issue Assigned: {self.title}"
            from_email = DEFAULT_FROM_EMAIL
            recipient_list = [self.assigned_to.email]
            
            context = {"issue": self, "site_name": SITE_NAME}
            
            html_email = render_to_string("email/issue_assignment_notification.html", context)
