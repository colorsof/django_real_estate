...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        html_email = render_to_string("email/issue_confirmation.html", context)
        ...
        logger.error(f"Failed to send issue confirmation email for issue {issue.title}: {e}",
                     exc_info=True)
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        context = { "issue": issue, "site_name": SITE_NAME }
        html_email = render_to_string("email/issue_resolved_notification.html", context)
        text_email = strip_tags(html_email)
        from_email = DEFAULT_FROM_EMAIL
        to=[issue.reported_by.email]
        email = EmailMultiAlternatives(subject, text_email, from_email, to)
        ...
        logger.error(f"Failed to send issue resolution email for issue {issue.title}: {e}",
                     exc_info=True)
