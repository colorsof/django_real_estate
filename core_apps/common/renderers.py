...
class GenericJSONRenderer(JSONRenderer):
    """
    A custom JSON renderer that standardizes the API response format.
    """
    ...
    def render(self, data: Any, accepted_media_type: Optional[str] = None, 
        ...
        if not response:
            raise ValueError("Response object is not found in renderer_context")
        ...
        return json.dumps({
            ...
            object_label: data}).encode(self.charset)  