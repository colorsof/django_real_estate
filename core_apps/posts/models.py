...
class Post(TimeStampedModel):
    ...
    def get_popular_tags(cls, limit=5):
        return cls.tags.annotate(post_count=Count(
            "taggit_taggeditem_items")).order_by("-post_count")[:limit]
        
        # return cls.tags.annotate(post_count=Count('post')).order_by("-post_count")[:limit]
        
    def save(self, *args, **kwargs) -> None:
        if not(self.author.is_superuser or self.author.is_staff or 
               self.author.profile.occupation == Profile.Occupation.TENANT):
            raise ValueError("Only tenancts, superusers or staff members can create posts")
