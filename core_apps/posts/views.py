...
class PostDetailAPIView(generics.RetrieveAPIView):
    ...
    object_label = "post"
    lookup_field = "slug"   #is this a search parameter or what
    ...
    def get_object(self) -> Post:
        ...
        filter_kwargs = {self.lookup_field: self.kwargs[self.lookup_field]}   #what is this magic
        post = generics.get_object_or_404(queryset, **filter_kwargs)
        self.record_post_view(post)  #more magic
        return post
    ...
    def record_post_view(self, post):
        ...
        obj, created = ContentView.objects.update_or_create(   #what exactly are we doing here
...
class PostUpdateAPIView(generics.UpdateAPIView):
        ...
        def update(self, request, *args, **kwargs):
            ...
            return Response(response_data)  #wow what was all that????????
...
class UnBookmarkPostAPIView(APIView):
        def patch(self, request, slug):
            ...
            if user not in post.bookmarked_by.all():
                return Response(
                    {
                        "message": "You can't remove a bookmark that does not exist!"
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            post.bookmarked_by.remove(user)
            return Response({"message": "Post Bookmark Removed"}, 
...
class DownvotePostAPIView(APIView):
    def patch(self, request, post_id):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Post downvoted succesfully"},
                status=status.HTTP_200_OK
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
