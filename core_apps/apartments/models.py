...
class Apartment(TimeStampedModel):
    ...
    tenant = models.ForeignKey(
        User, 
        on_delete=models.SET_NULL, #sets apartment.tenant to null if user is deleted thus not        
        null=True,   blank=True,   #models.CASCADE which would delete the apartment
        related_name="apartment", #Used to access user's apartments eg user.apartments.all()
